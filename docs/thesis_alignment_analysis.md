...
## 3. Liturgical Alignment Validation
...
### 3.1. Liturgy Alignment (The Temple Stack)
...
**Engine Implementation (`resolve_liturgy_hymns`)**:
Validated in `test_L_master_temple_stack_sunday_saint`, the engine compiles exactly these 7 components in the precise order specified:
...
### 3.2. Isodikon Logic (Liturgical Refrains)
...
**Engine Implementation (`resolve_isodikon`)**:
Validated in `test_L_isodikon_logic`, the engine dynamically yields the correct refrain string by checking the context rank and calendar date.

---

### 3.3. Anaphora Type Selection
...
**Engine Implementation (`resolve_anaphora_type`)**:
Validated in `test_L_anaphora_basil`, the engine resolves the proper Liturgy variant, mapping Lenten Sundays to the Basil Anaphora, and ordinary days to Chrysostom.

---

### 3.4. Koinonikon Stack (Communion Hymns)
...
**Engine Implementation (`resolve_koinonikon_stack`)**:
Validated in `test_L_koinonikon_stack`, the engine stacks the Sunday Koinonikon ("Praise the Lord from the heavens...") and the Saint's Koinonikon ("In everlasting remembrance...") in hierarchical sequence.

---

### 3.5. Matins Canon Ratios
...
**Engine Implementation (`resolve_canon_ratio`)**:
Validated in `test_M_canon_ratio`, the engine dynamically calculates the ratio parameters according to the feast rank, returning the exact `4 / 2 / 2 / 4` partition.

---

### 3.6. Matins Praises Ratios
...
**Engine Implementation (`resolve_matins_praises_ratio`)**:
Validated in `test_M_praises_ratio`, the engine splits the praises slots into the correct `4 / 4` ratio.
